...
(($) => {
	...
	Twinkle.block.callback = () => {
		if (
			...
			!confirm(
				wgULS('您即将对自己执行封禁相关操作！确认要继续吗？', '您即將對自己執行封鎖相關操作！確認要繼續嗎？')
			)
		) {
			return;
		}
	...
	Twinkle.block.callback.change_action = (e) => {
		...
		if (templateBox) {
			...
			const $previewlink = $(`<a id="twinkleblock-preview-link">${wgULS('预览', '預覽')}</a>`);
								...
								list: [
									{
										type: 'option',
										value: 'blocked',
										label: `blocked - ${wgULS('仅依行为证据认定', '僅依行為證據認定')}`,
										selected: true,
									},
									{
										type: 'option',
										value: 'proven',
										label: `proven - ${wgULS('经傀儡调查确认', '經傀儡調查確認')}`,
									},
									{
										type: 'option',
										value: 'confirmed',
										label: `confirmed - ${wgULS('经查核确认', '經查核確認')}`,
									},
								],
							},
							{
		...
		if (field_block_options) {
			...
			$form.find('[name=pagerestrictions]').select2({
				...
				language: {
					errorLoading: () => wgULS('搜索词汇不完整或无效', '搜尋詞彙不完整或無效'),
				},
			...
			mw.util.addCSS(
				 ...
				 */
				'.select2-results .select2-results__option{padding-top:1px;padding-bottom:1px}.select2-container .select2-dropdown .select2-results,.select2-container .selection .select2-selection__rendered{font-size:13px}.select2-container--default.select2-container--focus .select2-selection--multiple{border:1px solid #aaa}.select2-selection__choice__remove{font-size:130%}'
			);
		} else {
		...
		if (Twinkle.block.hasBlockLog) {
			...
			if (Twinkle.block.currentBlockInfo) {
				...
				if (lastBlockAction.action === 'unblock') {
					...
					// block or reblock
					blockloginfo.push(
						`，${new Morebits.date(blockAction.params.expiry).calendar('utc')}${wgULS('过期', '過期')}`
					);
				}
			}
		...
		if (toBlock) {
			...
			api.get(query).then((data) => {
				...
				if (user && Morebits.string.isInfinity(blockoptions.expiry)) {
					groupsToBeRemoved = user.groups.filter((group) => groupsCanBeRemoved.includes(group));
				}
				// execute block
	...
	Twinkle.block.callback.main = (pageobj) => {
		...
		if (Twinkle.getPref('blankTalkpageOnIndefBlock') && params.template !== 'uw-lblock' && params.indefinite) {
			Morebits.status.info(
				wgULS('信息', '資訊'),
				wgULS('根据参数设置清空讨论页并为日期创建新2级标题', '根據偏好設定清空討論頁並為日期建立新2級標題')
			);
